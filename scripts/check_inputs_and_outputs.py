...
def checkInputFile(filepath: str):
    ...
    try:
        ...
        if len(lines) != n+1:
            return "invalid number of lines in file"
        for i in range(1, n+1):
            line = list(map(int, lines[i].split()))
            m = line[0]
            if m < 0 or m > n-1:
                return f"m is out of bounds on line {i+1}"
            if len(line) != m+1:
                return f"invalid number of edges on line {i+1}"
            for x in line[1:]:
                if x == i:
                    return f"self edge on line {i+1}"
                if x < 1 or x > n:
                    return f"out of bounds edge on line {i+1}"
                if (i, x) in edges:
                    return f"duplicate edge on line {i+1}"
                edges.add((i, x))
    except:
...
def checkOutputFile(filepath: str):
    ...
    try:
        ...
        if n == 0:
            ...
            if len(removed) != n:
                return "invalid amount of removed vertices"
            if len(removed) != len(set(removed)):
                return "duplate vertices"
    except:
        return "invalid file formatting"
    finally:
        file.close()
...
for folder in INPUT_FOLDERS:
    for file in os.listdir(folder):
        ...
        if not os.path.isfile(filepath):
            continue
        if len(filepath) < 4 or filepath[-4:] != ".txt":
            continue
