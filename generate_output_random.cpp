// This is for testing and for comparing how optimal our current solution is
// It works the same as our main idea, it just removes a random node each time instead of the max weight
...
int main(int argc, char* argv[]){
    ...
    while (!sccForest.empty()) {
        ...
        int nodeToRemove;
        int index = rand() % scc.size(), i = 0;
        for (auto kv : scc){
            if (i == index){
                nodeToRemove = kv.first;
                break;
            }
            i++;
        }
